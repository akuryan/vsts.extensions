...
function CollectOutBoundIpAddresses {
    ...
    $resources = Get-AzureRmResource -ResourceGroupNameContains $resourceGroupName
...
function CollectWebAppOutboundIpAddresses{
    ...
    if (!$resourcePresenceChecked) {
        ...
        $webAppResource = (Get-AzureRmResource -ResourceGroupNameContains $resourceGroupName).where({$_.Name -eq "$webAppName" -And $_.ResourceGroupName -eq "$resourceGroupName"})
...
function SetWebAppRestrictions {
    ...
    #get all resrouces in current resource group, and check if resource is present there
    $webAppResource = (Get-AzureRmResource -ResourceGroupNameContains $resourceGroupName).where({$_.Name -eq "$webAppInstanceName" -And $_.ResourceGroupName -eq "$ResourceGroupName"})
    #measure found amount and if less or equal to 0 - we could not find web app
    if (($webAppResource | Measure-Object).Count -le 0) {
